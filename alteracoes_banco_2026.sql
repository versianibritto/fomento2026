...
UPDATE `fases` SET `deleted` = '1' WHERE (`id` = '6');
UPDATE `fases` SET `deleted` = '1' WHERE (`id` = '7');
...
SELECT 
    ...
    `p`.`resultado` AS `resultado`,
    DATE_FORMAT((`p`.`created` + INTERVAL 3 HOUR), '%d/%m/%Y') AS `created`,
    DATE_FORMAT((`p`.`data_inicio` + INTERVAL 3 HOUR), '%d/%m/%Y') AS `data_inicio`,
    DATE_FORMAT((`entrada`.`data_inicio` + INTERVAL 3 HOUR), '%d/%m/%Y') AS `primeira_bolsa`,
    `p`.`vigente` AS `vigente`,
    `p`.`tipo_bolsa` AS `tipo_bolsa`,
    `p`.`justificativa_cancelamento` AS `justificativa_cancelamento`,
    DATE_FORMAT((`p`.`deleted` + INTERVAL 3 HOUR), '%d/%m/%Y') AS `deleted`,
    ...
    DATE_FORMAT((`p`.`homologado_data` + INTERVAL 3 HOUR), '%d/%m/%Y') AS `homologado_data`,
    `h`.`nome` AS `homologado_por`,
    `p`.`homologado_justificativa` AS `homologado_justificativa`
FROM
    `projeto_bolsistas` `p`
    LEFT JOIN `editais` `ed` ON (`ed`.`id` = `p`.`editai_id`)
    LEFT JOIN `usuarios` `u` ON (`u`.`id` = `p`.`bolsista`)
    LEFT JOIN `usuarios` `h` ON (`h`.`id` = `p`.`homologado_por`)
    LEFT JOIN `streets` `s` ON (`u`.`street_id` = `s`.`id`)
    LEFT JOIN `districts` `d` ON (`d`.`id` = `s`.`district_id`)
    LEFT JOIN `cities` `c` ON (`c`.`id` = `d`.`city_id`)
    LEFT JOIN `states` `st` ON (`st`.`id` = `c`.`state_id`)
...
VIEW `dashyodacounts` AS
    ...
    WHERE
        ((`pb`.`deleted` IS NULL)
            AND (`e`.`fim_vigencia` > NOW()))
    GROUP BY pb.homologado, `pb`.`fase_id` , `f`.`bloco` , `pb`.`vigente` , `e`.`programa_id` , `pb`.`editai_id`;


CREATE  OR REPLACE 
VIEW `dashcountavaliadores` AS
    SELECT 
        `ab`.`usuario_id` AS `usuario_id`,
        `u`.`nome` AS `usuario_nome`,
        `ab`.`ano` AS `ano`,
        `ab`.`situacao` AS `situacao`,
        `ab`.`deleted` AS `deleted`,
        COUNT(DISTINCT `ab`.`id`) AS `qtd_inscricoes`
    FROM
        (`avaliador_bolsistas` `ab`
