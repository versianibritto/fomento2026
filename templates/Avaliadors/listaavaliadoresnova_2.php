...
'O'=>'Renovação não concluída'];?>
    ...
    <h4>Lista de Avaliadores NOVA</h4>

    <?php if($this->request->getAttribute('identity')['yoda']){?>
        <div class="well">
            <?=$this->Form->create(null,['url'=>['controller'=>'Avaliadors','action'=>'listaavaliadoresnova']]);?>
                <div class="input-group">
                    ...
                    <span class="input-group-btn">
                        <?=$this->Html->link(' Todas as unidades  <i class="fas fa-eye"></i>', ['controller'=>'Avaliadors','action' => 'listaavaliadoresnova/1'], ['class' => 'btn btn btn-info mr-1', 'escape' => false])?>
                    </span>
                </div>
            <?=$this->Form->end();?>
        </div>
            ...
            <table class="table table-hover text-nowrap">
                ...
                <tbody>
                    ...
                    <tr style="<?=(($b->situacao=='F'?'color:#090!important':''))?>">
                        ...
                        <td><?=$this->Html->link($b->bolsista.'   <i class="fas fa-eye"></i>', ['controller'=>'Projetos','action' => 'detalhesubprojeto', $b->bolsista], ['class' => 'btn btn-xs btn-info mr-1', 'escape' => false])?></td>
                        <td><?=$nome_b[0] . ' ' . end($nome_b) ?></td>
                        <td><?=$nome_o[0] . ' ' . end($nome_o) ?></td>
                        <td><?=$b->projeto_bolsista->orientadore->unidade->sigla?></td>
                        <td>
                            <?php if($b->projeto_bolsista->orientador!=$this->request->getAttribute('identity')['id']){?>
                                <?=$nome[0] . ' ' . end($nome) ?>
                            <?php }?>
                            <?php if($b->projeto_bolsista->orientador==$this->request->getAttribute('identity')['id']){?>
                                <?='XXX' ?>
