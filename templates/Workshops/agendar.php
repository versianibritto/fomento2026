<?php
$tipoApresentacaoAtual = (string)($workshop->tipo_apresentacao ?? '');
$localAtual = (string)($workshop->local_apresentacao ?? '');
$dataAtual = !empty($workshop->data_apresentacao) ? $workshop->data_apresentacao->format('Y-m-d') : '';
?>

<div class="container mt-4">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        <h4 class="mb-0"><?= $jaAgendada ? 'Reagendar' : 'Agendar' ?> Workshop #<?= (int)$workshop->id ?></h4>
        <?= $this->Html->link('Voltar', ['controller' => 'Workshops', 'action' => 'ver', $workshop->id], ['class' => 'btn btn-sm btn-outline-secondary']) ?>
    </div>
        ...
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <?= h((string)($workshop->usuario->nome ?? 'Não informado')) ?>
                </div>
                <div class="col-md-4">
                    <strong>Orientador</strong><br>
                    <?= h((string)($workshop->orientadore->nome ?? 'Não informado')) ?>
                </div>
                <div class="col-md-4">
                    <strong>Edital Workshop</strong><br>
                    <?= h((string)($workshop->editai->nome ?? ('#' . (int)$workshop->editai_id))) ?>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            <?= $this->Form->create($workshop) ?>
                ...
                <div class="mt-4 d-flex gap-2">
                    ...
                    <?= $this->Html->link('Cancelar', ['controller' => 'Workshops', 'action' => 'ver', $workshop->id], ['class' => 'btn btn-outline-secondary']) ?>
                </div>
            <?= $this->Form->end() ?>
        </div>
    </div>
