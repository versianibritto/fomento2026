<div class="container mt-4">
    <div class="py-2 mb-3 border-bottom text-center">
        <?= $this->element('pdj_inscricoes_steps', [
            'edital' => $edital,
                ...
                <div class="card-body">
                    ...
                    <?= $this->Form->create(null, ['type' => 'file']) ?>
                        ...
                        <fieldset class="anexos-areas">
                            ...
                            <?php if (!empty($inscricao->bolsista) && !empty($bolsistaSexoFeminino) && !empty($anexoTipoFilhosMenorBolsista)) : ?>
                                <?php
                                    $filhosBolsistaValor = (int)($inscricao->filhos_menor_bolsista ?? 0);
                                    $tipoFilhosBolsistaId = (int)$anexoTipoFilhosMenorBolsista->id;
                                    $rotuloFilhosBolsista = (string)$anexoTipoFilhosMenorBolsista->nome;
                                ?>
                                <div class="mt-4">
                                    <div class="row g-3 align-items-start">
                                        <div class="col-md-6">
                                            <?= $this->Form->control('filhos_menor_bolsista', [
                                                'label' => 'Bolsista, possui filhos menores de 8 anos?',
                                                'type' => 'select',
                                                'options' => [
                                                ...
                                                'empty' => 'Selecione',
                                                'class' => 'form-control',
                                                'id' => 'filhos-menor-bolsista',
                                                'value' => $inscricao->filhos_menor_bolsista ?? null,
                                            ]) ?>
                                        </div>
                                        <div class="col-md-6">
                                            <div
                                                class="row g-3 anexos-areas"
                                                id="bloco-anexo-filhos-bolsista"
                                                style="display: <?= $filhosBolsistaValor > 0 ? 'flex' : 'none' ?>;"
                                            >
                                                <div class="col-12">
                                                    <?php if (empty($anexos[$tipoFilhosBolsistaId])) : ?>
                                                        <?= $this->Form->control("anexos[$tipoFilhosBolsistaId]", [
                                                            'type' => 'file',
                                                            'label' => h($rotuloFilhosBolsista),
                                                            'class' => 'form-control',
                                                        ]) ?>
                                                    <?php else : ?>
                                                        <label class="form-label d-block"><?= h($rotuloFilhosBolsista) ?></label>
                                                        <div class="anexo-arquivo-atual">
                                                            <div class="d-flex align-items-center justify-content-between gap-2 flex-nowrap">
                                                                <div class="small text-muted text-truncate">
                                                                    <?= h($anexos[$tipoFilhosBolsistaId]) ?>
                                                                </div>
                                                                <div class="d-inline-flex align-items-center gap-1 flex-nowrap">
                                                                    <a href="/uploads/anexos/<?= h($anexos[$tipoFilhosBolsistaId]) ?>" target="_blank" class="btn btn-light border btn-sm py-0 px-2" title="Download">
                                                                        <i class="fa fa-download"></i>
                                                                    </a>
                                                                    <label for="anexo-<?= (int)$tipoFilhosBolsistaId ?>" class="btn btn-light border btn-sm py-0 px-2 mb-0" title="Alterar">
                                                                        <i class="fa fa-edit"></i>
                                                                    </label>
                                                                    <button
                                                                        type="button"
                                                                        class="btn btn-light border-danger text-danger btn-sm py-0 px-2 mb-0"
                                                                        onclick="confirmarExclusaoAnexo(<?= (int)$tipoFilhosBolsistaId ?>, this.form)"
                                                                        title="Excluir"
                                                                    >
                                                                        <i class="fa fa-trash"></i>
                                                                    </button>
                                                            ...
                                                            <input
                                                                id="anexo-<?= (int)$tipoFilhosBolsistaId ?>"
                                                                name="anexos[<?= (int)$tipoFilhosBolsistaId ?>]"
                                                                type="file"
                                                                class="d-none anexo-file"
                                                                data-tipo="<?= (int)$tipoFilhosBolsistaId ?>"
                                                            >
                                                        </div>
                                                    <?php endif; ?>
                                                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const selectFilhosBolsista = document.getElementById('filhos-menor-bolsista');
    const blocoAnexoFilhosBolsista = document.getElementById('bloco-anexo-filhos-bolsista');
    if (selectFilhosBolsista && blocoAnexoFilhosBolsista) {
        selectFilhosBolsista.addEventListener('change', function () {
            const mostrar = parseInt(selectFilhosBolsista.value || '0', 10) > 0;
            blocoAnexoFilhosBolsista.style.display = mostrar ? 'flex' : 'none';
            if (!mostrar) {
                const inputFile = blocoAnexoFilhosBolsista.querySelector('input[type="file"]');
                if (inputFile) {
                    inputFile.value = '';
                }
            }
...
<?= $this->element('pdj_form_empty_feedback') ?>
